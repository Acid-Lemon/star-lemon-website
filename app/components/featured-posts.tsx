    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {posts.map((post, index) => (
        <Link key={post.id} href={`/post/${post.id}`}>
          <article className="group h-full bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 hover:-translate-y-2">
            {/* 文章头部装饰 */}
            <div className="relative h-48 bg-gradient-to-br from-gray-50 to-gray-100 overflow-hidden">
              {post.cover ? (
                ...
                <>
                  ...
                  <div className="absolute bottom-4 left-4 w-16 h-16 rounded-full bg-blue-200/30 blur-xl" />
                  
                  {/* 文章编号 */}
                  <div className="absolute top-6 left-6">
                    <span className="text-5xl font-bold text-gray-200 group-hover:text-orange-200 transition-colors duration-300">
            ...
            <div className="p-6">
              ...
              <div className="flex items-center text-sm text-orange-500 font-medium opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0 transition-all duration-300">
                阅读全文
