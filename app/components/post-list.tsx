                ...
                {posts.map((post, index) => {
                    ...
                    return (
                       <Link href={`/post/${post.id}`} className="block group" key={`post-${post.id}`}>
                            <article className="relative flex flex-col sm:flex-row items-start gap-6 p-6 bg-white dark:bg-gray-900 rounded-2xl shadow-sm group-hover:shadow-xl transition-all duration-300 group-hover:-translate-y-1 border border-gray-100 dark:border-gray-800 overflow-hidden">
