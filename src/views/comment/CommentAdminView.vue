        ...
        <div class="w-full h-[95vh] bg-[#F8FAFD] flex flex-col content-center items-center">
            <el-tabs v-model="active_name" class="w-[95%]" @tab-click="handle_click">
                <el-tab-pane label="全部" name="all">
                    <div class="w-full">
                        <el-table :data="message_list[current_page - 1]" :row-style="(row) => {return row.row.public_state ? '--el-table-tr-bg-color: var(--el-color-success-light-9)' : '--el-table-tr-bg-color: var(--el-color-warning-light-9)'}" border
                                  max-height="80vh"
                                  style="width: 100%">
                            <el-table-column type="index" width="50"/>
                            <el-table-column label="留言id" prop="id" width="250"/>
                            <el-table-column label="内容" prop="content"/>
                            <el-table-column :formatter="(row) => {return row.public_state ? '可见' : '不可见'}" label="可见状态"
                                             prop="public_state"
                                             width="100"/>
